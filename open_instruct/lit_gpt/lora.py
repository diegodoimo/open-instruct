...
class LoRAQKVLinear(LoRALinear):
    ...
    def forward(self, x: torch.Tensor, idx: Optional[int] = None) -> torch.Tensor:
        ...
        #     torch.save(pretrained, "./results/base_qkv_lit.pt")
        #     torch.save(x, "./results/norm1_inner_lit.pt")
        #     torch.save(self.linear.weight, "./results/base_weight_inner_lit.pt")
        #     torch.save(self.linear.bias, "./results/base_bias_inner_lit.pt")
...
class GPT(BaseModel):
    ...
    def forward(
        ...
        # torch.save(x, "./results/embd_lit.pt")
        for i, block in enumerate(self.transformer.h):
            x = block(x, cos, sin, mask, input_pos, idx=i)
            # if i == 0:
            #     #    print("first_block:", x)
            #     torch.save(x, "./results/first_block_lit.pt")

        x = self.transformer.ln_f(x)
        # print("last_hidden:", x)
        # torch.save(x, "./results/last_hidden_lit.pt")
        if lm_head_chunk_size > 0:
            # chunk the lm head logits to reduce the peak memory used by autograd
            return [self.lm_head(x_i) for x_i in x.split(lm_head_chunk_size, dim=1)]
        x = self.lm_head(x)  # (B, T, vocab_size)
        # torch.save(x, "./results/logits_lit.pt")
    ...
    def get_output_lora_B(self):
        return self.lm_head.lora_B

    def set_output_lora_B(self, lora_b):
        self.lm_head.lora_B = lora_b
